...
const dataList = computed(() =>
  Array.from({ length: listNum.value }, () => props.list)
);
...
function changeTransition(time?: number | string) {
  ...
  if (typeof time === 'number' || typeof time === 'undefined') {
    wrapperRef.value.style.transition = `transform ${
      time ?? props.speed
    }ms linear`;
    return;
  }
...
function intervalRolling() {
  if (!value.value || listNum.value === 1) return;
  nums = getListHeight();
  handleRolling();
...
function wheelScrollEvent(direction: DIRECTION) {
  ...
  if (direction === DIRECTION.DOWN && currentIndex.value > nums.length - 2)
    currentIndex.value = 0;
...
function wheelScroll(e: WheelEvent) {
  if (!props.hover || !props.wheel || listNum.value === 1) return;
  if (e.deltaY > 0) wheelScrollEvent(DIRECTION.DOWN);
  else wheelScrollEvent(DIRECTION.UP);
}
...
function initScroll() {
  currentIndex.value = 0;
  stopScroll();
  if (wrapperRef.value) wrapperRef.value.style.transform = `translateY(0px)`;
}
...
const getOverlapHeight = debounce(() => {
  ...
  listNum.value =
    (listRef.value[0]?.clientHeight ?? 0) - root.clientHeight <= 0 ? 1 : 2;
  nextTick(() => {
    if (value.value && listNum.value > 1) startScroll();
    if (listNum.value === 1) initScroll();
  });
}, 200);
